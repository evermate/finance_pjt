...
class InterestOption(models.Model):
    ...
    intr_rate_type = models.CharField(max_length=1)  # S: 단리, M: 복리 등
    intr_rate_type_nm = models.CharField(max_length=20)
    save_trm = models.CharField(max_length=10)  # 단위: 개월

    # 적금 전용 필드 (정액/자유)
    rsrv_type = models.CharField(max_length=1, null=True, blank=True)
    rsrv_type_nm = models.CharField(max_length=20, null=True, blank=True)

    intr_rate = models.FloatField(null=True, blank=True)   # 기본금리
    intr_rate2 = models.FloatField(null=True, blank=True)  # 최고우대금리
